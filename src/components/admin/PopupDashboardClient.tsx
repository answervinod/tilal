...
export function PopupDashboardClient({ initialInquiries }: { initialInquiries: any[] }) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/popup-inquiry/${id}`, { method: 'DELETE' });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const deletePromises = Array.from(selectedIds).map(id => 
        fetch(`/api/popup-inquiry/${id}`, { method: 'DELETE' }).then(res => {
                ...
                {filteredInquiries.map((inq, index) => {
                  ...
                  return (
                    <tr key={inq.id} className={`hover:bg-slate-50/50 transition-colors group ${selectedIds.has(inq.id) ? 'bg-blue-50/30' : ''}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-slate-900 font-medium">{inq.nationality}</div>
                      </td>
                ...
                {filteredInquiries.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-6 py-16 text-center text-slate-500">
                      <p className="text-base font-medium text-slate-900">No popup inquiries found</p>
