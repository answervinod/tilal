  ...
  return (
    <div className="relative flex" ref={wrapperRef}>
      ...
      <button
        ...
        className={`flex items-center gap-2 px-3 py-3 bg-slate-50 border border-r-0 text-sm transition-colors ${
          error ? 'border-red-500' : 'border-neutral-300'
        } focus:outline-none focus:border-brand`}
      >
        <span className="text-base leading-none">{selectedCountry.flag}</span>
        <span className="text-slate-600 font-medium whitespace-nowrap">{selectedCountry.dialCode}</span>
      ...
      <input
        ...
        className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-brand transition-colors ${
          error ? 'border-red-500' : 'border-neutral-300'
        }`}
