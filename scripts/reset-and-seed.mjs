/**
 * Reset: delete all page docs, then re-seed them cleanly with `language` set from the start.
 * This fixes the document-internationalization slug-uniqueness conflict.
 */
...
async function deleteAllPages() {
  const ids = await client.fetch(`array::compact(*[_type == "page"]._id)`);
  if (!ids.length) return;
  console.log(`Deleting ${ids.length} existing page docs...`);
...
(async () => {
  ...
  const docs = [
    ...
    makeHomePage({ id: 'page-home-en', title: 'Home', slug: 'home', locale: 'en' }),
    makeHomePage({ id: 'page-home-ar', title: 'الرئيسية', slug: 'home', locale: 'ar' }),
  ];
