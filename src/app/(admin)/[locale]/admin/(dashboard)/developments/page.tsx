...
export default async function AdminDevelopmentsDashboard() {
  ...
  const { data: enquiries, error } = await supabase
    .from('enquiries')
    .select('*')
    .neq('project_slug', 'WhatsApp Contact')
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden">
      <div className="flex items-center justify-between px-8 py-5 bg-white/40 backdrop-blur-sm border-b border-slate-200/50 sticky top-0 z-10">
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Development Page Enquiries</h1>
      </div>
