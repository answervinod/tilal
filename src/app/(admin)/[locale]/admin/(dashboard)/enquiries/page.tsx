...
export default async function AdminEnquiriesDashboard() {
  ...
  if (error) {
    // If the table doesn't exist yet, show a friendly error
    const isTableMissing = error.message.includes('relation "enquiries" does not exist');
    return (
      <div className="p-6 bg-red-50 border-l-4 border-red-500 rounded-md m-8">
        <h3 className="text-red-800 font-medium">Database Error</h3>
        <p className="text-red-700 text-sm mt-2">
          {isTableMissing 
            ? 'The "enquiries" table does not exist in Supabase yet. Please run the provided SQL script to create it.'
            : error.message}
        </p>
      </div>
    );
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden">
      <div className="flex items-center justify-between px-8 py-5 bg-white/40 backdrop-blur-sm border-b border-slate-200/50 sticky top-0 z-10">
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">PDF Enquiries</h1>
      </div>
